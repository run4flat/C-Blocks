...
It will soon be possible in my fork of tcc to compile a file and obtain
the list of all tokens that expect to be externally linked. This will
allow me to write an libperl.xs.PL file that produces a libperl.xs file
which loads the cached symbol table and adds all symbols. The resulting
C code should be properly linked to libperl, whether libperl is static
or dynamic, and so it should be possible to get libperl to work for
everyone.
...
Also, the current copy and cache procedures use a home-spun binary tree
to map old pointers to new pointers. While this achieves the goal of
reducing the time complexity of the copy and serialization process, I'm
sure I can use less memory and write a faster implementation with a hash
table.
