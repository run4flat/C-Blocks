L<https://travis-ci.org/run4flat/C-Blocks.svg?branch=master>
...
=item Interpolation blocks on Windows

These are currently failing on my Windows machine.

=item Better representation on CPAN Testers

Very few machines report this module as working on CPAN testers,
including nothing for a number of key platforms.
...
machine (The "nearly" business is that interpolation blocks do not seem
to work on my Windows machine), and I have implemented the functionality
and tests I had hoped to implement in the Alpha stage. The present goal
is to find and fix all the ways that C::Blocks can break, i.e. expand
