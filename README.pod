...
This distribution seems to be working very well for me. However, even 
if the current version passes its test suite and installs on your 
system, it may be subtly broken and throw segmentation faults during 
ordinary use. Uncovering and fixing these is one of the major goals of
the Alpha stage. You can be more confident in the distribution's
robustness once it reaches Beta.

Currently this distribution builds and passes its test suite on at 
least a subset of the Linux, Mac, and Windows machines reporting on 
CPAN Testers, and I have implemented the functionality and many of the 
tests I had hoped to implement in the Alpha stage. Now that I have 
passing reports on the Big Three, I turn my attention to expanding the
test suite.
...
The C<cisa> keyword implements a very simple type system, introduced 
based on experience durinig the alpha stage. In practice, the current 
implementation is awkward to use. I would like to redesign the type 
system during Beta.
