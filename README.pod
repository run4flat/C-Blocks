...
Note that the next numeric note should be #6.

=over

=item 3

On Mac OSX 10.7 (Lion) and on Debian Wheezy running on BeagleBone Black (and
possibly on Deiban Wheezy on i386 hardware as well), running test 10 with
C<prove> results in the following error:

 Internal error: unable to locate extended symbol

On the BBB, this error occured both with C<prove> and with C<perl -Mblib>.
