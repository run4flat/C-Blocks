...
This project is nearing its alpha release, at which point I spin a CPAN
distribution and see what happens. (See the goals and milestones below.)
Using this software in its current state may cause segmentation faults
and the API is likely to change.

=head1 DEVELOPMENT BOOTSTRAPPING

Both C<C::Blocks> and the extended symbol table management fork of the
Tiny C Compiler are under active and symbiotic development. Once the
basic set of features are in place, I will create two distributions,
C<Alien::TinyCCx> for the compiler and C<C::Blocks> for this
distribution. At the current stage of development, coordinating both of
those moving targets would be a pain. To that end, this distribution in
its current form is unconventional. It is not set up as a typical CPAN
distribution, but is instead set up to help developers and early
testers get started as quickly and painlessly as possible. It will
automatically make a temporary installation of the most recent development
version of the Tiny C Compiler with extended symbol table support. The
temporary installation will be in a directory within your C::Blocks build
directory. This is all performed when you run C<Build.PL>.

=head2 Basic Fast Setup

The fastest way to get set up and see if things work is to checkout the
C<cached-names> branch like so:

 git clone https://github.com/run4flat/C-Blocks.git
 cd C-Blocks
 git checkout cached-names
 perl Build.PL

This should quickly provide an incomplete but functional interface to
libperl.

=head2 Basic Full Setup

If the cached-names branch fails to compile, or if you want to try
parts of the Perl API that are not available via the cached names branch,
run the same set of commands on the master branch:

 git clone https://github.com/run4flat/C-Blocks.git
 cd C-Blocks
 perl Build.PL

Be warned, however, that the verification process can take a very long
time. On my Linux machine it took 10 minutes. On my Windows machine, it
took 30 minutes. If you just want to get started, see the fast branch
discussed above, or the no-caching branch discussed next.

=head2 No Caching

If you encounter trouble with caching, you can also try the C<libperl-nocache>
branch. This will not cache the symbol table from F<perl.h> at all. However,
it will also fail to load any linker symbols (i.e. functions) if your
libperl happened to be compiled as a static library (which is often the
case). Don't be surprised, then, if F<t/04-libperl.t> fails if you check
out this branch.

=head2 Environment Variable Setup

There is only one other thing you will have to do: you will set the C<LD_LIBRARY_PATH> (Unixen)
...
=head2 Temporary Branches

The F<cached-names> and F<libperl-nocache> are merely for your convenience.
They are not development branches. I will frequently rebase these branches
onto the latest master branch.
...
=item Get libperl caching working on all platforms

The symbol verification process is slow, but that is not the key issue
with libperl caching at the moment. The problem is that on Windows, I get
a long list of linker errors, stating C<undefined reference to `_imp__...'>.
These need to be figured out. (Speeding up the symbol verification would
also be nice.)
...
In particular, I would like to see a collection of tests for the Perl C
API.
...
=item Streamline Symbol Table Copying and Caching

This is really an issue for the underlying libtcc implementation, but it
will be a blocker for C::Blocks, as far as I am concerned.

The symbol
table copy procedure currently copies I<every> token and Sym, which come
along for the ride with the caching procedure. This is a poor solution
to an important problem. A large number of tokens are added to each
compiler context before any compilation begins, including tokens like
C<str>, C<push>, and C<pop>. If a compiling context has a function
defined with this name, it will refer to a token that was in place before
the compiler saw one character of code. Thus, if I only copy tokens that
were created by the compilation unit, I could miss some of these tokens.
Thus I copy all TokenSyms. Instead, I should just copy (and serialize)
the TokenSyms that are used. This will reduce the memory footprint of
extended symbol tables and the quantity of data read from disk.
...
utilize the symbol table load/dump functionality and symbol verification
currently under development for use with libperl. Once it's working for libperl,
I want to create author tools for L<Module::Build> and L<ExtUtils::MakeMaker> so
that the symbol table cache can be created at distribution build time.
